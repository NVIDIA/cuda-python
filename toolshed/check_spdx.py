...
LICENSE_IDENTIFIER_REGEX = re.compile(
    re.escape(SPDX_LICENSE_IDENTIFIER_PREFIX) + rb"(?P<license_identifier>[^\r\n]+)"
)
...
def extract_license_identifier(blob):
    ...
    if match is None:
        return None
    license_identifier = (
        match.group("license_identifier").decode("ascii", errors="replace").strip()
    )
    for comment_suffix in ("-->", "*/"):
        if license_identifier.endswith(comment_suffix):
            license_identifier = license_identifier.removesuffix(
                comment_suffix
            ).rstrip()
...
def validate_license_identifier(filepath, blob):
    ...
    expected_license_identifier, configuration_error = get_expected_license_identifier(
        filepath
    )
...
def find_or_fix_spdx(filepath, fix):
    ...
    good = True
    has_license_identifier = validate_required_spdx_field(
        filepath, blob, SPDX_LICENSE_IDENTIFIER_PREFIX
    )
    has_copyright = validate_required_spdx_field(
        filepath, blob, SPDX_FILE_COPYRIGHT_TEXT_PREFIX
    )
