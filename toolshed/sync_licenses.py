...
def sync_licenses(root: Path, fix: bool) -> int:
    ...
    mismatches = [
        relative
        for relative in PACKAGE_LICENSES
    ...
    print(
        "Run toolshed/sync_licenses.py --fix to update package copies.", file=sys.stderr
    )
    return 1
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check package license files against the canonical root LICENSE."
    )
    parser.add_argument(
        "--fix", action="store_true", help="replace mismatched package copies"
    )
