...
COMPONENT_TO_TAG_RE: dict[str, re.Pattern[str]] = {
    ...
    "cuda-pathfinder": re.compile(
        rf"^cuda-pathfinder-v(?P<version>{_VERSION_PATTERN})$"
    ),
}
...
def check_release_notes(
    git_tag: str, component: str, repo_root: Path = Path(".")
    ...
    if version is None:
        return [
            (
                "<tag>",
                f"cannot parse version from tag '{git_tag}' for component '{component}'",
            )
        ]
...
def warn_missing_backport_notes(
    git_tag: str, component: str, problems: list[tuple[str | Path, str]]
) -> None:
    ...
    for path, reason in problems:
        print(
            f"::warning file={path}::Release notes for backport tag {git_tag} are {reason}."
        )
        print(f"  - {path} ({reason})")
    ...
    summary_lines.extend(
        [
            "",
            "Please add the backport release notes on `main` if they are not already present.",
        ]
    )
    write_step_summary("\n".join(summary_lines))
...
def validate_backport_decision(
    *,
    ...
    if backport_branch is None:
        print(
            "ERROR: cannot determine backport branch from ci/versions.yml or GITHUB_REF_NAME.",
            file=sys.stderr,
        )
        return 2, []
    ...
    if is_backport_version(version, backport_branch):
        ...
        if problems:
            ...
            print(
                f"Release notes present for backport tag {git_tag}, component {component}."
            )
        return 0, []
    ...
    if decision == BACKPORT_NOT_PLANNED:
        print(
            f"Backport release not planned for {git_tag}, skipping backport release-notes check."
        )
        return None, []
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--component", required=True, choices=list(COMPONENT_TO_PACKAGE)
    )
    ...
    if rc is not None:
        if problems:
            print(
                f"ERROR: release notes policy failed for tag {args.git_tag}:",
                file=sys.stderr,
            )
            for path, reason in problems:
    ...
    if not problems:
        print(
            f"Release notes present for tag {args.git_tag}, component {args.component}."
        )
        return 0

    print(
        f"ERROR: missing or empty release notes for tag {args.git_tag}:",
        file=sys.stderr,
    )
    for path, reason in problems:
        print(f"  - {path} ({reason})", file=sys.stderr)
