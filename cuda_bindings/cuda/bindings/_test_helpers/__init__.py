...
The idea here is to import the current cuda_python_test_helpers that cuda_bindings
expects, even if the real cuda_python_test_helpers name is already claimed by an older,
incompatible copy.

.. deprecated:: cuda-core-1.2
    Remove this entire subpackage once cuda-core >= 1.2 is the oldest supported
    release.
"""
...
def _find_current_test_helpers_package():
    """Locate and load cuda_python_test_helpers source checkout under a private
    alias name, bypassing sys.modules entirely.
    """
...
def _import_current(submodule_name: str):
    ...
    ``cuda_python_test_helpers`` name. Falls back to a modern copy loaded under
    a private alias if the real name resolves to an older, incompatible copy --
    e.g. released cuda-core <= 1.1.1 (tagged 2026-07-29, six days before #2384
    landed) ships its own ``tests/conftest.py`` that puts a pre-#2384
    ``cuda_python_test_helpers`` (predating arch_check.py / mempool.py /
    pep723.py living there at all) on sys.path and caches it in sys.modules
    under the real name before this shim ever runs.
    """
    try:
