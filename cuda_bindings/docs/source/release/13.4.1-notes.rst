...
* Fixed a bug in the wrapping of ``nvrtcBundledHeadersInfo``.
  (`PR #2754 <https://github.com/NVIDIA/cuda-python/pull/2754>`_)
* Fixed ``CUDA_PYTHON_DISABLE_MAJOR_VERSION_WARNING``: previously, setting it
  to ``"0"`` (or any other non-empty string) still disabled the warning; it
  is now parsed as a bool-like value, so ``"0"`` correctly leaves the warning
  enabled.
