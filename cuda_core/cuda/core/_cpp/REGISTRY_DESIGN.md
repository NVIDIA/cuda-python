...
(e.g., `CUevent`, `CUkernel`, `CUgraph`, `CUgraphNode`) to a `weak_ptr`
for its owning resource handle. When a `_ref` constructor receives a raw handle, it
...
`kernel_registry`, `graph_registry`, `graph_node_registry`.
