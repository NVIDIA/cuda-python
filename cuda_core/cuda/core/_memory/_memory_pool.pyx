...
# cumemlocation_from_type is referenced only from CUDA 13 branches. cython-lint
# does not evaluate compile-time IF blocks, so it needs a pragma to be seen as used.
from cuda.core._memory._location cimport cumemlocation_from_type  # no-cython-lint
...
cdef int MP_init_current_pool(
    ...
    IF CUDA_CORE_BUILD_MAJOR >= 13:
        ...
        cdef cydriver.CUmemLocation loc = cumemlocation_from_type(loc_type, loc_id)
        with nogil:
