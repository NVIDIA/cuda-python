...
def _import_versioned_module() -> None:
    """Select the build for the installed cuda-bindings, after checking it is supported.
    ...
    header it compiled against and its cuda-bindings floor in ``_build_info``
    (generated by build_hooks.py). The installed cuda-bindings must be of the
    build's major, at least as new as the floor, and generated from a
    ``cuda.h`` at least as new as the build's (see
    ``_bindings_floor.check_installed_bindings``), or import fails here with an
    actionable message instead of later with a missing C function or a
    ...
    def load_build_module(name: str, cuda_major: int):
        # Prefer this major's build in the merged wheel; fall back to a plain build.
        try:
