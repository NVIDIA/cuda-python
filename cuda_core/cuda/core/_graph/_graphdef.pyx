...
This module provides GraphDef and a GraphNode class hierarchy for building CUDA
graphs explicitly (as opposed to stream capture). Both approaches produce
the same public Graph type for execution.

GraphNode hierarchy:
    GraphNode (base — also used for the internal entry point)
