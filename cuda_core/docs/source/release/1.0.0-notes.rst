...
- :attr:`Linker.backend` is available as an instance property for querying the
  backend selected for a constructed linker. ``Linker.which_backend()`` remains
  available as a deprecated compatibility classmethod for default non-LTO PTX
  backend queries, but emits a ``DeprecationWarning`` because backend selection
  is now per linker instance.
