...
- The path-valued :class:`ProgramOptions` fields (``include_path``,
  ``pre_include``, ``create_pch``, ``use_pch``, ``pch_dir``,
  ``fdevice_time_trace``) now accept any :class:`os.PathLike` in addition to
  ``str``, and normalize what they are given to :class:`pathlib.Path`. Code
  that builds paths with :mod:`pathlib` no longer has to convert back to
  ``str``.
  (`#2500 <https://github.com/NVIDIA/cuda-python/pull/2500>`__)
