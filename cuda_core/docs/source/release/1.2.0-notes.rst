...
- Resource objects now report their live state through ``bool(obj)`` and reject
  active operations after they are closed. Cross-object APIs also reject closed
  streams, events, buffers, contexts, memory pools, graph objects, and texture
  resources before passing a null handle to CUDA. Closing either default-stream
