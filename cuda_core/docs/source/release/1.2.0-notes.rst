...
- Python subclasses of pool-backed memory resources and
  :class:`GraphMemoryResource` now route buffer teardown from
  :meth:`MemoryResource.allocate` through :meth:`MemoryResource.deallocate`,
  matching :meth:`Buffer.from_handle` behavior. Built-in resource types keep
  the direct C++ deallocation path for interpreter-shutdown safety.
  (`#2615 <https://github.com/NVIDIA/cuda-python/issues/2615>`__)
