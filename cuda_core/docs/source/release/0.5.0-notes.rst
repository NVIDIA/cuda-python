...
- Added memory mangment support (allocation, deallocation, copy, and fill) for CUDA graphs.
...
The support for setting :attr:`VirtualMemoryResourceOptions.handle_type` to `"win32"` is removed. Please reach out to us on GitHub if you have a real-life use case.
...
- :class:`StridedMemoryView` ``__init__`` constructor is deprecated in favor of ``from_*`` classmethods, see below.
...
- Added :meth:`Buffer.fill` method for efficient memory initialization, supporting ``int``, ``bytes``,  general buffer protocol objects, and CUDA graphs.
