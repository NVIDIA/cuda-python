...
* `cuda.coop <https://nvidia.github.io/cccl/python/cooperative>`_: A Python module providing CCCL's reusable block-wide and warp-wide *device* primitives for use within Numba CUDA kernels
* `cuda.compute <https://nvidia.github.io/cccl/python/parallel>`_: A Python module for easy access to CCCL's highly efficient and customizable parallel algorithms, like ``sort``, ``scan``, ``reduce``, ``transform``, etc. that are callable on the *host*
