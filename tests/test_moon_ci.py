...
class TestMoonCi:
    ...
    def test_public_automation_roots_are_nvidia_only(self) -> None:
        workflow = read(".github/workflows/ci.yml")
        assert workflow.count("    if: ${{ github.repository_owner == 'nvidia' }}") == 3
        assert "    if: ${{ always() && github.repository_owner == 'nvidia' }}" in workflow
