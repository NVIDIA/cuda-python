...
def resolve_conda_anchor(desc: HeaderDescriptor, conda_prefix: str | os.PathLike[str]) -> Path | None:
    ...
    *anchor_point*, or ``None`` if the conda layout is not usable.
    """
    if IS_WINDOWS:
        anchor = Path(conda_prefix, "Library")
        return anchor if anchor.is_dir() else None
    if desc.conda_targets_layout:
        targets_include_paths = glob.glob(str(Path(conda_prefix, "targets", "*", "include")))
        if len(targets_include_paths) != 1:
            return None
        return Path(targets_include_paths[0]).parent
