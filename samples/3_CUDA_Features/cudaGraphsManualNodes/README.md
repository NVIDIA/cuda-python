...
[`samples/cudaGraphs/`](../../cudaGraphs/), which teaches stream capture
at the `cuda.core` layer. This sample is the only place in `/samples`
...
- [`samples/cudaGraphs/`](../../cudaGraphs/) - the high-level `cuda.core` equivalent (stream capture only)
