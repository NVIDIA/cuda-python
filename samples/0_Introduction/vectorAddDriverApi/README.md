...
[`samples/vectorAdd/`](../../vectorAdd/) sample, which does the same
computation with the ``cuda.core`` API. Both are useful:

- Use ``samples/vectorAdd/`` when you want an idiomatic, Pythonic launch.
...
### From `cuda_bindings_utils` (samples/Utilities/)
...
- [`samples/vectorAdd/`](../../vectorAdd/) - the high-level `cuda.core` equivalent
